...
require_env AWS_KMS_KEY_ARN
# it should use KMS ARN to simplifly the test case because the describe-volumes returns the arn
# even when the kms id is passed as argument

@test "$DRIVER: Should Create an EBS Encrypted Instance Using specified Key" {
    #Use Instance Type that supports EBS Optimize
    run machine create -d amazonec2 --amazonec2-instance-type=t2.micro --amazonec2-volume-kms-key "$AWS_KMS_KEY_ARN" --amazonec2-volume-encrypted $NAME
    echo ${output}
    [ "$status" -eq 0 ]
}
...
@test "$DRIVER: Check the machine is up and EBS is encrypted" {
    ...
    run docker $(machine config $NAME) run --rm -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e AWS_DEFAULT_REGION=$AWS_DEFAULT_REGION -e AWS_ZONE=$AWS_ZONE -e AWS_VPC_ID=$AWS_VPC_ID amazon/aws-cli ec2 describe-volumes --filters Name=attachment.instance-id,Values="${instance_id}" --query 'Volumes[0].KmsKeyId' --output text
    kms_key=${lines[-1]}
    echo $kms_key
    [[ "$kms_key" == "$AWS_KMS_KEY_ARN" ]]
}
